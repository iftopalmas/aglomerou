...
exports.inserir = async (req, res) => {
  ...
  try {
    ...
    if (error.message.includes('usuario_email_key'))
      res
        .status(409)
        .send({ message: 'Já existe um usuário com o email informado!' });
...
exports.atualizar = async (req, res) => {
  ...
  try {
    ...
    if (resultado.rowCount > 0) {
      ...
      res
        .status(404)
        .send({ message: 'Usuário com o id informado não foi encontrado!' });
    }
  } catch (error) {
...
exports.deletar = async (req, res) => {
  ...
  try {
    ...
    if (resultado.rowCount > 0)
      ...
      res
        .status(404)
        .send({ message: 'Usuário com o id informado não foi localizado!' });
  } catch (error) {
    serverError(res, error);
  } finally {
    client.end();
